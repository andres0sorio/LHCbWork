...
//std::ofstream *os = new std::ofstream("data.out",std::ofstream::out);
...
double integrate( Ptr2Function func , double *x , double *pp , double xmin , double xmax ) 
{
  ...
  val = trapzd(fvalues, xmin, xmax);

  std::cout << "With trapezoid: " << val << std::endl;
  
  return val;
  
}

double integrate2d( Ptr2Function func , double *x , double *pp , double *xmin , double *xmax ) 
{
  
  //Extending integrate to two dimensions
...
double trapzd(std::vector<double>& y, double xlo, double xhi)
{
  ...
  if (n == 2) {
    return (s=0.5*(xhi-xlo)*(y[0]+y[1]));
    
  } else {
    
    del = (xhi-xlo) / (double) n;
    for (sum = 0.0, j=0; j < n ; j++) sum += y[j];
    s =  del * sum - ( 0.5 * del * ( y[0] + y[n-1]) ); 
...
// double qsimp(std::vector<double>& y, double xlo, double xhi)
// {
