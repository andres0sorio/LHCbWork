...
for i in `seq $MIN $MAX`;
  ...
  sed -e "s/set PARF=/set PARF=$FILE/" $INPUT > $OUTPUT
...
for i in `seq $MIN $MAX`;
  ...
  sed -e "s/Executable =/Executable = \"$script\";/;
          ...
          s/InputSandbox =/InputSandbox = \{\"$script\",\"$data\",\"$EXE\",\"setROOT.csh\",\"findROOT.sh\",\"$parfile\"\};/" $INPUT > $OUTPUT
